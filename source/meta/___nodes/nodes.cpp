...
namespace madX::Meta::Nodes
{
	...
	void Node::Draw(ImDrawList* draw_list, ImVec2 offset, float scale) const
	{
	...
	LinearRoot BuildLinearRootPaths(const Meta::Nodes::NodesContainer& nodes)
	{
		struct Depth
		{
		...
		LinearRoot result;
		Meta::Vector<Depth> linear;

		for (const auto& node : nodes)
		{
			if (false == (node->type_ & Meta::Nodes::NodeType_Root)) [[likely]]
				continue;
			...
			{
				const Depth& back = linear.back();

				if (back.path_ == back.begin_)
					result.emplace_back(depth, root);
				...
				assert(false == linear.empty());
				if (linear.erase(&linear.back()); false == linear.empty()) [[likely]]
				{
					depth--;

					if (auto& last = linear.back(); last.path_ != last.end_)
						last.path_++;
