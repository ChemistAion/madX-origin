...
namespace madX::Editor::Panel
{
	...
	class Sequencer final : public PanelClass
	{
		////////////////////////////////////////////////////////////////////////////////
		...
		struct Sequence : public ImSequencer::SequenceInterface
		{
			...
			{
				SequenceItem& item = myItems[index];
				if (color)
			...
			{
				myItems.push_back(SequenceItem { type, 0, 10, false });
			};
			...
			std::vector<SequenceItem> myItems;
			RampEdit rampEdit;
	...
	public:
		Sequencer()
		{
			sequence_.mFrameMin = -100;
			sequence_.mFrameMax = 1000;
			sequence_.myItems.push_back(Sequence::SequenceItem { 0, 10, 30, false });
			sequence_.myItems.push_back(Sequence::SequenceItem { 1, 20, 30, true });
			sequence_.myItems.push_back(Sequence::SequenceItem { 3, 12, 60, false });
			sequence_.myItems.push_back(Sequence::SequenceItem { 2, 61, 90, false });
			sequence_.myItems.push_back(Sequence::SequenceItem { 4, 90, 99, false });		
		}
		...
		void RenderPanel(const std::shared_ptr<Root::Component::Project>&) override
		{
			...
			ImGui::InputInt("Frame Min", &sequence_.mFrameMin);
			ImGui::SameLine();
			ImGui::InputInt("Frame ", &currentFrame);
			ImGui::SameLine();
			ImGui::InputInt("Frame Max", &sequence_.mFrameMax);
			ImGui::PopItemWidth();
			...
			if (selectedEntry != -1)
			{
				const Sequence::SequenceItem& item = sequence_.myItems[selectedEntry];
